  ...
  return (
    <article className="bg-void-900/60 border border-void-700 hover:border-matrix-500/40 px-3 py-2.5 transition-colors" data-testid={`digest-rule-${r.id}`}>
      ...
      <div className="mt-1.5 flex items-center gap-3 text-[11px] flex-wrap">
        ...
        {r.quality_score !== null && <span className="font-mono text-gray-600" title="Metadata completeness">completeness {r.quality_score}</span>}
    ...
    <div className="space-y-6">
      ...
      {data && (
        <>
          ...
          {/* Retired rules. Upstream deprecations arrive here as removals:
              every parser skips deprecated/ folders, so a rule moved there
              vanishes from the corpus and gets a tombstone (#87). */}
          <Panel
