...
QUERYABLE_FIELDS: list[FieldSpec] = [
    ...
    FieldSpec(
        ...
        kind="list",
        columns=["platforms"],
        description="Operating system the telemetry comes from: windows, linux, macos, container, cross_platform, not_applicable, unknown.",
        examples=["platform:windows", "platform:not_applicable"],
    ),
    FieldSpec(
        aliases=["domain"],
        kind="list",
    ...
    FieldSpec(
        aliases=["product", "vendor"],
        kind="list",
        columns=["products"],
        description="Vendor / application whose telemetry the rule reads (aws, okta, crowdstrike, sysmon, palo_alto, ...).",
        examples=["product:okta", "vendor:crowdstrike"],
    ),
    FieldSpec(
    ...
    FieldSpec(
        aliases=["network", "ioc", "indicator", "ip"],
        kind="list_substring",
        columns=["extracted_network_indicators"],
        description="Network indicator (IP, domain name, URL, port) the rule keys on. (domain: now means the attack-surface domain, #103.)",
        examples=["network:amazonaws.com", "ip:10.0.0"],
    ),
    FieldSpec(
