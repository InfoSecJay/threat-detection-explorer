...
| `source` | enum | One of `sigma`, `elastic`, `splunk`, `sublime`, `elastic_protections`, `lolrmm`, `elastic_hunting`, `sentinel`, `google_secops`, `okta_custom_detections`. |
...
| `updated_at` | `datetime` | Our sync timestamp — refreshed on every upsert. The atomic-swap cleanup keys on this. |
