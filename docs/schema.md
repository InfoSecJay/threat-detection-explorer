...
| --- | --- | --- | --- |
| `status` | enum | `stable`, `test`, `experimental`, `deprecated`, `unsupported`, `not_applicable`, `unknown` | Sigma's vocabulary, preserved 1:1 (issue #26): `test` = works but not field-proven, `unsupported` = cannot run on current tooling. Elastic `production` → `stable`, `development` → `experimental`. Panther/pypanther `Enabled: false` → `experimental` (disabled upstream). Sentinel templates `Available` → `stable` and `Experimental` → `experimental` (#137); a template with no `status` key is a published one → `stable` (#47). Sources with no lifecycle concept at all (Okta, Sublime, Elastic hunting / protections, Google SecOps) → `not_applicable` (#107), distinct from `unknown`, which is reserved for values we could not parse. `deprecated` comes from a vendor value or a deprecated-in-title marker (#109); parsers skip deprecated directories. Anything unrecognised → `unknown`. |
...
| `domains` | `list[str]` | Attack-surface domains (#103): `endpoint`, `identity`, `cloud`, `saas`, `network`, `email`, `devops`, `data`, `application`, or `unknown`. Derived from the raw platforms + data sources by `taxonomy/domains.py`; `platforms` itself is OS-only since 2026-09. |
...
| `equivalent_sources` | `list[str]` | Sources holding a "same behaviour" rule for this one: a shared observable (process, registry key, API action, path, indicator, event ID) plus a shared ATT&CK technique or a second shared observable. Recomputed by the post-sync batch (`app.services.equivalents`, #149); a value carried by more than 1% of the corpus never pairs two rules on its own. Powers the "Has equivalent in" facet, the `equivalent_in` / `no_equivalent_in` filters and the `equiv:` query field. Deprecated rules neither give nor get one. |
...
| `logic_hash` | `str?` | sha1 of the whitespace-normalized `detection_logic`, set at ingest (DX-14). Internal: not on the API. |
| `logic_changed_at` | `datetime?` | When a sync last saw `logic_hash` move (DX-14). A rule whose `rule_modified_date` is in a trending window but whose logic last changed before it was a metadata edit; `/trending/techniques` ranks on new + logic-changed rules and reports bulk commits separately. `null` until a change is observed. Internal: not on the API. |
...
| `status` | `status` | `Available` → `stable`, `Experimental` → `experimental`; missing → `stable` (published template) |
